      ...
      <div key={supply.id} className="border-b border-gray-100 last:border-0">
        <div className="flex items-center py-4 px-6 hover:bg-gray-50 transition-colors group">
          <div className="flex items-center gap-4 flex-1">
            <div className="p-2 bg-gray-50 rounded group-hover:bg-mg-red/10 transition-colors">
              <Package className="h-4 w-4 text-gray-400 group-hover:text-mg-red" />
            </div>
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-400 font-mono tracking-tighter">{supply.code}</span>
                <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest border-l border-gray-200 pl-2">
                  {supply.category}
                </span>
              </div>
              <span className="text-sm font-bold tracking-tight uppercase italic text-mg-black">
                {supply.description}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-12">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs font-bold text-mg-black italic">{supply.unit}</p>
            </div>
            <div className="text-right min-w-[100px]">
              <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Preço Unitário</p>
              <p className="text-sm font-black text-mg-red italic">R$ {supply.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
  ...
  return (
    <div id="supplies-manager" className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-end justify-between border-b border-gray-200 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-mg-black tracking-tighter uppercase italic">Gerenciador de <span className="text-mg-red">Insumos</span></h1>
          <p className="text-gray-500 text-sm font-medium mt-1 uppercase tracking-wider">Base de materiais, mão de obra e equipamentos.</p>
        </div>
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-1">
                  {['Material', 'Mão de Obra', 'Equipamento', 'Serviço', 'Outros'].map(cat => (
                    <label key={cat} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded cursor-pointer group">
                      ...
                      <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest group-hover:text-mg-black">{cat}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-mg-blue p-6 rounded shadow-lg text-white">
            <h4 className="text-[10px] font-black uppercase tracking-widest mb-2 flex items-center gap-2">
              <Info className="h-4 w-4 text-mg-red" /> Gestão de Insumos
            </h4>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-white p-4 border border-gray-100 shadow-sm flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-4 border-l border-gray-200">
               ...
               <span className="text-xs font-bold text-mg-black">5.420 Insumos</span>
            </div>
          </div>

          <div className="bg-white border-t-8 border-mg-red shadow-xl overflow-hidden">
            {supplies.map(supply => renderSupply(supply))}
          </div>
        </div>
      </div>
       ...
       <AnimatePresence>
        {isImportModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-8">
                <h3 className="text-xl font-black text-mg-black uppercase tracking-tighter italic mb-2">Importar <span className="text-mg-red">Insumos</span></h3>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-8">Selecione o arquivo de insumos para importação.</p>
                ...
                <div className="mt-8 space-y-4">
                  <div className="flex items-center gap-3 p-3 bg-mg-blue/5 rounded border border-mg-blue/10">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-mg-black uppercase tracking-widest leading-none">Formatos Suportados</p>
                      <p className="text-[9px] text-mg-blue font-medium uppercase tracking-wider mt-1 italic">Excel (.xlsx, .csv), JSON, XML</p>
                    </div>
                  </div>
                </div>
